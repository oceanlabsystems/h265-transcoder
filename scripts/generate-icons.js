...
try {
  ...
  console.error(
    "Error: sharp package is required. Install it with: npm install --save-dev sharp"
  );
  ...
  console.error(
    "   - Windows: https://convertio.co/svg-ico/ (create ICO with sizes: 16, 32, 48, 64, 128, 256)"
  );
  console.error(
    "   - Mac: https://cloudconvert.com/svg-to-icns (or use icon.png)"
  );
...
async function generateIcons() {
  try {
    ...
    try {
      ...
      console.log(
        "✓ Created icon.ico with multiple sizes (16, 32, 48, 64, 128, 256)"
      );
      icoCreated = true;
    } catch (e) {
      ...
      console.log(
        "  Install to-ico for proper ICO: npm install --save-dev to-ico"
      );
